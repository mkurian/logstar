...
public class AggregateTraces {
    ...
    private void process(File file, File outDir) throws IOException {
        ...
        }
        {
            ...
            //rootTrace.renderFilteredFriendsToHtmlList(out,2,true);
            {
    ...
    private List<MetaStackTrace> parceTraces(List<String> _lines) {
        ...
        //separate input lines flow into standalone traces
        ...
        for (String lineRaw : _lines) {
            ...
            if (current != null) {
                ...
                //here all plain log lines passed
            }
        }
        if (current != null) {
            resultList.add(current); //last but not least
        }

        //separate each standalone trace into nested traces
        for (MetaStackTrace meta : resultList) {
            meta.splitNestedTraces();
        }

        //find comlete friends set
        int progress = 0;
        for (MetaStackTrace meta : resultList) {
        ...
        //filter friends to make graph looks like tree but not exctly tree
...
class MetaStackTrace implements Comparable<MetaStackTrace> {
    ...
    public void splitNestedTraces() {
        if (this.isEmpty() == false) {
            ...
            items.add(current); //last but not least
        }
    }
    ...
    public String getLastExceptionHeader(){
        ...
        if (index > 0) {
            return header[2]; //nested trace, skip "Caused by:" - two words actually
        } else {
            return header[0];
        }
    }
    ...
    public String[] getSignature(String _rootPackage) {
        if ((signatureKey == null) || (signatureKey.equals(_rootPackage) == false)) {
            List<String> result = new ArrayList<String>(items.size() * 5); //exception, first project method, it's line, first method, line
            int index = items.size() - 1;
            while (index >= 0) {
                StackTrace trace = items.get(index);
                //parse first business method name, if exists
...
//                distance[i][j] = LevenshteinDistance.minimum(
//                        distance[i - 1][j] + 1,
//                        distance[i][j - 1] + 1,
//                        distance[i - 1][j - 1] + ((str1[i - 1].equals(str2[j - 1])) ? 0 : 1));

                //TODO find more appropriate weight for each change
                distance[i][j] = LevenshteinDistance.minimum(
                        distance[i - 1][j] + (1 + str1.length - i),
                        distance[i][j - 1] + (1 + str2.length - j),
                        distance[i - 1][j - 1] + ((str1[i - 1].equals(str2[j - 1])) ? 0 : Math.min((1 + str1.length - i), (1 + str2.length - j))));
